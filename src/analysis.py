...
def load_struct(args: argparse.Namespace) -> Tuple[mda.Universe, ag_type, list[transform_type]]:
    ...
    args.sel_superpos = get_cfg_sel(args.sel_superpos, args.cfg, args.selection)
    ...
    if(args.unwrap or args.wrap):
        ...
        if(args.unwrap_starters is None):
            ...
            unwrap_starters = utils.load_sel(args.unwrap_starters, unwrap_sel, indexes)
            print("Selected %d atoms as unwrap starters"%len(unwrap_starters))

    
    traj_transforms = []
    if(args.unwrap):
        traj_transforms.append(transformations.Unwrapper(unwrap_sel,unwrap_starters))
    
    if(args.wrap):
        print("Putting mol COMs back in box for selection of %d atoms"%len(unwrap_sel))
...
def analysis_subparser(parser: argparse.ArgumentParser) -> None:
    ...
    extr_parser.add_argument("--beginning", "-b", metavar="N",
                             ...
                             default=0,  type=int)
    extr_parser.add_argument("--unwrap",   action="store_true", help="Unwrap molecules (default: No)")
...
def extract_around(u: mda.Universe, sel: ag_type, transforms: list[transform_type], args: argparse.Namespace) -> dict[str,ArrayLike]:
    ...
    srt_ndx = np.arange(len(data["fval"]))[mask][np.argsort(np.abs(data["fval"][mask]-args.around))][:args.number]
    for key in data:
